...
class WebpToolsComponent @AssistedInject internal constructor(
    ...
    fun setWebpUri(uri: Uri) {
        ...
        collectionJob = componentScope.launch {
            ...
            val collectedUris = mutableListOf<String>()
            webpConverter.extractFramesFromWebp(
                ...
                webpService.cacheExtractResult(uri.toString(), collectedUris)
            }.collect { nextUri ->
                if (isLoading) {
                    _isLoading.update { false }
                }
                collectedUris.add(nextUri)
        ...
        savingJob = componentScope.launch {
            ...
            when (val type = _type.value) {
                is Screen.WebpTools.Type.WebpToImage -> {
                    ...
                    val targetUris = convertedImageUris.filterIndexed { index, _ ->
                        index in positions
                    }
                    _left.value = targetUris.size
                    val results = mutableListOf<SaveResult>()
                    targetUris.forEachIndexed { index, uri ->
                        imageGetter.getImage(data = uri, originalSize = true)?.let { localBitmap ->
