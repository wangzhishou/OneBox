...
import com.t8rin.imagetoolbox.core.resources.icons.line.LineArrowCircleRight
    ...
    AnimatedContent(
        ...
        when (state) {
            is WallpapersResult.Success -> {
                Column {
                    if (isPortrait) {
                        Spacer(Modifier.height(AppTheme.dimens.spaceLarge))
                    }
                    QualitySelector(
                    ...
                    if (component.imageFormat.canChangeCompressionValue) {
                        Spacer(Modifier.height(AppTheme.dimens.spaceSmall))
                    }
                    ImageFormatSelector(
            ...
            is WallpapersResult.Failed.NoPermissions -> {
                Column(
                    verticalArrangement = Arrangement.spacedBy(AppTheme.dimens.spaceExtraSmall)
                ) {
                    if (isPortrait) {
                        Spacer(Modifier.height(AppTheme.dimens.spaceSmall))
                    }
            ...
            is WallpapersResult.Loading -> Unit
        }
    }
}
...
private fun PermissionItem(
    ...
    PreferenceItem(
                ...
                Permission.ReadMediaImages -> {
                    if (Build.VERSION.SDK_INT >= Build.VERSION_CODES.TIRAMISU) {
                        context.requestPermissions(
                            permissions = listOf(Manifest.permission.READ_MEDIA_IMAGES).toTypedArray()
                        )
                    }
                }
            }
        ...
        endIcon = com.t8rin.imagetoolbox.core.resources.Icons.Outlined.LineArrowCircleRight,
...
private val Permission.icon: ImageVector
    get() = when (this) {
        Permission.ManageExternalStorage -> com.t8rin.imagetoolbox.core.resources.Icons.Rounded.FolderOpened
        Permission.ReadMediaImages -> com.t8rin.imagetoolbox.core.resources.Icons.Outlined.LineImageSearch
    }
