...
internal class BaiduBosAdapter : ObjectStorageVendorAdapter {
    override val bucketInPath: Boolean = true
    private val signer = BaiduBosSigner()

    override fun buildListBucketsRequest(connection: CloudStorageConnection.S3Compat): Request {
        val endpoint = normalizeEndpoint(connection)
        val target = ObjectStorageRequestFactory.RequestTarget(
    ...
    override fun buildCopyObjectRequest(
        ...
        bucket = bucket,
        key = targetKey,
        headers = mapOf("x-amz-copy-source" to "/$bucket/$sourceKey"),
    )
    ...
    override fun buildSignedGetUrl(
        ...
        val normalizedConnection = connection.copy(endpoint = "https://${normalizeEndpoint(connection)}")
        return signer.presign(normalizedConnection, "GET", createTarget(normalizedConnection, bucket, key), expiresInSeconds).toString()
    }
    ...
    override fun signedRequest(
        ...
        val target = createTarget(connection.copy(endpoint = "https://${normalizeEndpoint(connection)}"), bucket, key, query)
        val payloadHash = body?.let { SignatureUtils.sha256Hex(it) } ?: BaiduBosSigner.EMPTY_SHA256
        val signedHeaders = signer.signHeaders(connection, method, target, query, headers, payloadHash)
        val builder = Request.Builder()
            .url(target.url)
            .header("Host", target.host)
    ...
    private fun normalizeEndpoint(connection: CloudStorageConnection.S3Compat): String {
        val ep = connection.endpoint.removePrefix("https://").removePrefix("http://")
        return if (ep.startsWith("s3.")) ep else "s3.$ep"
    }
}
