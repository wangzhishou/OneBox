...
internal class AliyunOssAdapter : ObjectStorageVendorAdapter {
    override val bucketInPath: Boolean = false
    private val signer = AliyunOssSigner()
    ...
    override fun buildListBucketsRequest(connection: CloudStorageConnection.S3Compat): Request {
        ...
        val target = "https://$endpoint/".toHttpUrl()
        val requestTarget = com.wanbaohe.cloud.storage.data.protocol.ObjectStorageRequestFactory.RequestTarget(
            url = target,
            pathForSigning = "/",
            host = endpoint,
        )
        val headers = signer.signHeaders(connection, "GET", requestTarget, emptyMap(), emptyMap())
    ...
    override fun buildDeleteObjectRequest(connection: CloudStorageConnection.S3Compat, bucket: String, key: String): Request =
        signedRequest(connection, "DELETE", bucket, key = key)
    ...
    override fun buildCopyObjectRequest(
        ...
        bucket = bucket,
        key = targetKey,
        headers = mapOf("x-oss-copy-source" to "/$bucket/$sourceKey"),
    )
