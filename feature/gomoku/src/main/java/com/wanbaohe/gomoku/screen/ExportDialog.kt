    ...
    AlertDialog(
        ...
        text = {
            Column(verticalArrangement = Arrangement.spacedBy(12.dp)) {
                    ...
                    if (onShareImage != null) {
                        GlassTonalButton(
                            ...
                            if (isSharingImage) {
                                ...
                                Text(stringResource(R.string.gomoku_share_image))
                            }
                        }
                    }
                }
                ...
                Text(
                    text = exportContent.ifBlank { stringResource(R.string.gomoku_empty_export) },
                    ...
                    Row(horizontalArrangement = Arrangement.spacedBy(8.dp)) {
                        GlassTonalButton(
                            onClick = {
                                Clipboard.copy(
                                    text = exportContent,
                                    message = R.string.gomoku_copied,
                                )
                            },
                            modifier = Modifier.weight(1f),
                        ) {
                            Text(stringResource(R.string.gomoku_copy_notation))
                        }
                        GlassTonalButton(
                            onClick = { context.shareText(exportContent) },
                            modifier = Modifier.weight(1f),
                        ) {
                            Text(stringResource(R.string.gomoku_share_notation))
                        }
                    }
                }
            }
        ...
        confirmButton = {
            GlassTonalButton(onClick = onDismiss) {
                Text(stringResource(R.string.gomoku_close))
            }
        },
    )
}
