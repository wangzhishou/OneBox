...
import com.t8rin.imagetoolbox.feature.settings.data.keys.ACTIVE_THEME_ID
...
import com.t8rin.imagetoolbox.feature.settings.data.keys.CUSTOM_BACKGROUND_IMAGE_URI
import com.t8rin.imagetoolbox.feature.settings.data.keys.CUSTOM_BACKGROUND_OVERLAY_ALPHA
...
import com.t8rin.imagetoolbox.feature.settings.data.keys.LIQUID_GLASS_ENABLED
...
import com.t8rin.imagetoolbox.feature.settings.data.keys.MESH_GRADIENT_BACKGROUND_ENABLED
...
internal class AndroidSettingsManager @Inject constructor(
    ...
    override val settingsState: StateFlow<SettingsState> = rawFlow()
        .onEach {
            syncStartupSettingsToMmkv(it)
    ...
    /**
     * 将启动关键字段同步到 MMKV 缓存。
     * 在 settingsState Flow 每次 emit 时调用（包括首次加载 + 后续变更）。
     */
    private fun syncStartupSettingsToMmkv(state: SettingsState) {
        AppSharedStorage.syncStartupSettings(
            fontScale = state.fontScale,
            nightMode = state.nightMode.ordinal,
            isDynamicColors = state.isDynamicColors,
            appColorTuple = state.appColorTuple,
            ...
            isSystemBarsVisibleBySwipe = state.isSystemBarsVisibleBySwipe,
    ...
    override suspend fun toggleGlassmorphismEnabled() = toggle(
        key = GLASSMORPHISM_ENABLED,
        defaultValue = default.isGlassmorphismEnabled
    )

    override suspend fun toggleLiquidGlassEnabled() = toggle(
        key = LIQUID_GLASS_ENABLED,
        defaultValue = default.isLiquidGlassEnabled
    )

    override suspend fun toggleMeshGradientBackgroundEnabled() = toggle(
        key = MESH_GRADIENT_BACKGROUND_ENABLED,
        defaultValue = default.isMeshGradientBackgroundEnabled
    )

    override suspend fun setCustomBackgroundImageUri(uri: String?) = edit {
        if (uri != null) {
            it[CUSTOM_BACKGROUND_IMAGE_URI] = uri
        } else {
            it.remove(CUSTOM_BACKGROUND_IMAGE_URI)
        }
    }

    override suspend fun setGradientBackgroundStyle(style: GradientBackgroundStyle) = edit {
        it[GRADIENT_BACKGROUND_STYLE] = style.ordinal2
    }

    override suspend fun setCustomBackgroundOverlayAlpha(alpha: Float) = edit {
        it[CUSTOM_BACKGROUND_OVERLAY_ALPHA] = alpha.coerceIn(0f, 1f)
    }

    override suspend fun setActiveThemeId(id: String) = edit {
        it[ACTIVE_THEME_ID] = id
    }

    override suspend fun setGlassBaseAlpha(alpha: Float) = edit {
