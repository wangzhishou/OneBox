...
object LunarCalendarCalculator {
    ...
    // 日干支:经典算法以 1900-01-31(甲辰日)为基准 +40;dd() 以 1900-01-01 为基准,故 +30+40=+70
    private const val DAY_GANZHI_OFFSET = 70
    ...
    // LI 为经典 lunarInfo 编码:低 4 位=闰月月份,bit16=闰月长短,bit(16-m)=第 m 月长短
    private fun leapMonth(y: Int): Int { val i = y - 1900; return if (i < 0 || i >= LI.size) 0 else (LI[i] and 0xF).toInt() }
    ...
    private fun monthDays(y: Int, m: Int): Int { val i = y - 1900; return if (i < 0 || i >= LI.size) 29 else if ((LI[i] shr (16 - m)) and 1L == 1L) 30 else 29 }
    ...
    // 月干支:正月干 = (年干序号%5)*2+2(甲己丙寅、乙庚戊寅、丙辛庚寅、丁壬壬寅、戊癸甲寅),逐月顺推;正月支固定为寅
    fun getGanZhiMonth(ly: Int, lm: Int): String { val yg = ((ly - 4) % 10 + 10) % 10; return TIAN_GAN[((yg % 5) * 2 + 1 + lm) % 10] + DI_ZHI[(lm + 1) % 12] }
    fun getGanZhiDay(y: Int, m: Int, d: Int): String { val o = dd(y, m, d) + DAY_GANZHI_OFFSET; return TIAN_GAN[((o % 10) + 10) % 10] + DI_ZHI[((o % 12) + 12) % 12] }
    fun getDayGanIndex(y: Int, m: Int, d: Int): Int { val o = dd(y, m, d) + DAY_GANZHI_OFFSET; return ((o % 10) + 10) % 10 }
    fun getDayZhiIndex(y: Int, m: Int, d: Int): Int { val o = dd(y, m, d) + DAY_GANZHI_OFFSET; return ((o % 12) + 12) % 12 }
