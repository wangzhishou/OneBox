...
private fun Header(onDismiss: () -> Unit) {
    Row(
        ...
        Text(
            text = stringResource(R.string.chess_online_match_title),
...
private fun MatchContent(
    ...
    var mySide by remember { mutableStateOf(Side.WHITE) }
    ...
    val remotePlayerName = stringResource(R.string.chess_player_remote)
    val playerFallbackName = stringResource(R.string.chess_player_you)
    val createFailedText = stringResource(R.string.chess_create_room_failed)
    val joinFailedText = stringResource(R.string.chess_join_room_failed)
    val fetchFailedText = stringResource(R.string.chess_fetch_rooms_failed)
...
private fun ModeSelector(
    ...
    Row(
        ...
        ModeButton(
            label = stringResource(R.string.chess_create_room),
            isSelected = selected == MatchMode.CREATE,
            onClick = { onSelect(MatchMode.CREATE) },
            modifier = Modifier.weight(1f),
        )
        ModeButton(
            label = stringResource(R.string.chess_join_room),
...
private fun CreateRoomPanel(
    ...
    val inviteChooserTitle = stringResource(R.string.chess_online_invite_share_title)
            ...
            runCatching {
                ...
                shareProvider.shareImage(
                    imageInfo = ImageInfo(
                        ...
                        originalUri = "chess_online_invite_$roomCode",
                    ),
                    image = bitmap,
        ...
        when {
            currentRoom == null -> {
                OutlinedTextField(
                    ...
                    modifier = Modifier.fillMaxWidth(),
                    label = { Text(stringResource(R.string.chess_online_initial_fen)) },
                    supportingText = { Text(stringResource(R.string.chess_online_initial_fen_hint)) },
                    minLines = 3,
                )
                Button(
                    ...
                    if (isLoading) {
                        ...
                        Text(stringResource(R.string.chess_create_room_action))
                    }
                }
            }
            ...
            connectionState == ConnectionState.CONNECTING -> {
                ...
                Text(stringResource(R.string.chess_connecting))
            }

            connectionState == ConnectionState.WAITING_FOR_OPPONENT -> {
                val roomCode = currentRoom.id.takeLast(6)
                val invitationLink = remember(currentRoom.id) { buildChessJoinRoomDeeplink(currentRoom.id) }
                val invitationText = stringResource(
                    R.string.chess_online_invite_share_message,
                    roomCode,
                    invitationLink,
                )
                Text(
                    stringResource(R.string.chess_waiting_opponent),
                    style = MaterialTheme.typography.bodyLarge,
                )
                Text(
                    stringResource(R.string.chess_room_code, roomCode),
                    style = MaterialTheme.typography.bodySmall,
            ...
            connectionState == ConnectionState.PLAYING -> {
                Text(
                    stringResource(R.string.chess_opponent_ready),
                    style = MaterialTheme.typography.bodyLarge,
        ...
        Column(
            ...
            Surface(
                ...
                Column(
                    ...
                    Text(
                        text = stringResource(R.string.chess_online_invite_qr_title),
                    ...
                    Text(
                        text = stringResource(R.string.chess_room_code, roomCode),
            ...
            Row(
                ...
                OutlinedButton(
                    ...
                    if (isSharingImage) {
                        ...
                        Text(stringResource(R.string.chess_online_invite_share_image_action))
                    }
                }
                OutlinedButton(
                    onClick = onShareLink,
                    ...
                    Text(stringResource(R.string.chess_online_invite_share_link_action))
                }
            }
            OutlinedButton(
                onClick = {
                    Clipboard.copy(
                        text = invitationLink,
                        message = R.string.chess_online_invite_link_copied,
                    )
                },
                modifier = Modifier.fillMaxWidth(),
            ) {
                Text(stringResource(R.string.chess_online_invite_copy_link_action))
            }
        }
    }
}
...
private fun JoinTargetRoomPanel(
    ...
    Column(
        ...
        Text(
            text = stringResource(R.string.chess_join_target_room_title),
            style = MaterialTheme.typography.titleMedium,
            fontWeight = FontWeight.SemiBold,
        )
        Text(
            text = stringResource(R.string.chess_room_code, roomId.takeLast(6)),
            style = MaterialTheme.typography.bodyMedium,
            color = MaterialTheme.colorScheme.onSurfaceVariant,
        )
        if (shouldShowProgress) {
            CircularProgressIndicator(modifier = Modifier.size(40.dp))
            Text(stringResource(R.string.chess_join_target_room_connecting))
        } else {
            OutlinedButton(onClick = onRetry, modifier = Modifier.fillMaxWidth()) {
                Text(stringResource(R.string.chess_join_target_room_retry))
            }
        }
    }
}
...
private fun JoinRoomPanel(
    ...
    Column(modifier = Modifier.fillMaxWidth()) {
        Row(
            ...
            Text(
                stringResource(R.string.chess_room_list),
                style = MaterialTheme.typography.titleMedium,
            )
        ...
        if (isLoading && rooms.isEmpty()) {
            ...
            Box(
                ...
                Text(
                    stringResource(R.string.chess_no_rooms),
                    color = MaterialTheme.colorScheme.onSurfaceVariant,
                )
            }
        } else {
    ...
    Card(
        ...
        Row(
            ...
            Column(modifier = Modifier.weight(1f)) {
                ...
                Text(
                    stringResource(R.string.chess_room_code, room.id.takeLast(6)),
                    style = MaterialTheme.typography.bodySmall,
                    color = MaterialTheme.colorScheme.onSurfaceVariant,
                )
            }
            Button(onClick = onJoin) {
                Text(stringResource(R.string.chess_join))
            }
        }
    }
}
...
private fun buildChessJoinRoomDeeplink(roomId: String): String {
...
private fun shareChessRoomInvitation(
    context: Context,
    chooserTitle: String,
