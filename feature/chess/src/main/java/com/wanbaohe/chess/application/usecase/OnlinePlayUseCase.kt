 ...
 * Game-agnostic; caller provides gameType (国际象棋固定 "chess").
 *
 ...
 *
 * 国际象棋走子载荷:fromFile/fromRank -> toFile/toRank(即 UCI "e2e4" 的数值形态);
 * 升变不带目标子信息,接收端在 legalMoves 匹配时默认升后(见 GameComponent)。
 */
@Singleton
class OnlinePlayUseCase @Inject constructor(
    ...
    private inner class OnlineSession(
        ...
        // 白方(先手/房主侧)奇数 seq,黑方偶数 seq,两端命名空间隔离
        private fun nextOutgoingSeq(): Int {
            seqCounter += 1
            return when (mySide) {
                Side.WHITE -> seqCounter * 2 - 1
