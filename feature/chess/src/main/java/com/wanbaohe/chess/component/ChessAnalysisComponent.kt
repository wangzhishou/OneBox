...
data class ChessAnalysisUiState(
    val title: String = "",
    val boardState: BoardState = FenCodec.parse(FenCodec.INITIAL_FEN),
    val currentPly: Int = 0,
    val maxPly: Int = 0,
    val plies: List<ChessPlyRecord> = emptyList(),
    val exportContent: String = "",
    /**
     * 落库的结果码（[com.wanbaohe.chess.domain.GameResultCode]），与回放进度无关，整局恒定。
     *
...
class ChessAnalysisComponent @AssistedInject constructor(
    ...
    var uiState by mutableStateOf(ChessAnalysisUiState())
        private set
    ...
    init {
        ...
        componentScope.launch {
            ChessAudioDefaults.ALL.forEach { audioPlayer.warmUp(it) }
        }
        componentScope.launch {
    ...
    private fun playReplaySound(plies: List<ChessPlyRecord>, targetPly: Int) {
