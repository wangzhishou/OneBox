        ...
        onSuccess = { uri: Uri ->
            if (uri.isApng(context)) {
                ...
                AppToastHost.showToast(
                    ...
                    icon = com.t8rin.imagetoolbox.core.resources.Icons.Rounded.Apng
                )
            }
        }
    )
    ...
    AdaptiveLayoutScreen(
        ...
        actions = {
            ...
            ShareButton(
                ...
                onShare = {
                    component.performSharing(AppToastHost::showConfetti)
                },
                onEdit = {
        ...
        noDataControls = {
            val imageToApng = Screen.ApngTools.Type.ImageToApng()
            val apngToImage = Screen.ApngTools.Type.ApngToImage()
            TypeSelectionGrid(
                items = listOf(
                    TypeSelectionItem(imageToApng.title, imageToApng.subtitle, imageToApng.icon),
                    TypeSelectionItem(apngToImage.title, apngToImage.subtitle, apngToImage.icon)
                ),
                onClick = { index ->
                    when (index) {
                        0 -> imagePicker.pickImage()
                        1 -> pickSingleApngLauncher.pickFile()
                    }
                }
            )
        },
