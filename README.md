...
Wanbaohe 是一个基于 `ImageToolbox` 二次开发的多模块 Android 应用，围绕图片处理、AI 能力、文件与文档工具，以及日常效率功能进行扩展。项目使用 Jetpack Compose 构建 UI，使用 Decompose 管理运行时导航与页面组件，并通过 Hilt、KSP 与自定义 convention plugins 维护模块化工程结构。
...
当前根目录 `README.md` 仅作为项目正式入口文档维护；历史命令、维护者草稿和零散笔记已移出，避免与当前工程结构混淆。

## 安全与第三方服务说明

- **第三方密钥不随源码分发**：支付（微信/支付宝）、AI 服务商、Google Places 等密钥全部由可选的 `keystore.properties` 注入或保存在服务端，仓库中没有可用于正式环境的密钥。
- **源码中的静态 API Token**（`core/r/**/UrlConstantsFlavor.kt` 中的 `ACCESS_TOKEN` / `REMOTE_CONFIG_ACCESS_TOKEN`）是 App 内置的只读游客凭证，用于同步公开内容（清单、AI 引擎配置、远程配置等），与每个已发布 APK 中携带的相同。它们不能用于支付、评论写入或 AI 聊天等需要登录的接口。
