...
// 输出目录
const outDir = path.resolve(__dirname, '../../feature/markdown-edit/src/main/assets/milkdown-editor');
...
// KaTeX CSS 单独打包（按需加载）
const katexCSSBuildOptions = {
    entryPoints: [path.resolve(__dirname, 'katex-styles.css')],
    bundle: true,
    outfile: path.resolve(outDir, 'katex.bundle.css'),
    minify: !isWatch,
    logLevel: 'info',
    nodePaths: [path.resolve(__dirname, 'node_modules')],
    loader: {
...
async function build() {
    try {
        if (isWatch) {
            ...
            const mainStats = fs.statSync(mainBuildOptions.outfile);
            console.log(`✓ Built editor.bundle.js (${(mainStats.size / 1024).toFixed(2)} KB)`);

            // 构建 KaTeX CSS
            await esbuild.build(katexCSSBuildOptions);
            const katexStats = fs.statSync(katexCSSBuildOptions.outfile);
            console.log(`✓ Built katex.bundle.css (${(katexStats.size / 1024).toFixed(2)} KB) [按需加载]`);
        }
    } catch (error) {
