package com.shifenmiao.model.chess

/**
 * 国际象棋业务 Service 接口。
 *
 * 供 UI Component 层与 AI AgentTool 层共同依赖的契约。
 * 实现类位于 feature/chess/service/。
 */
interface ChessServiceInterface {

    /** 列出所有未归档的对局摘要（按最近对局时间倒序）。 */
    suspend fun listGames(): List<ChessGameSummaryDto>

    /** 获取单局详情（含走子记录），不存在返回 null。 */
    suspend fun getGameDetail(gameId: String): ChessGameDetailDto?

    /** 创建本地双人对局，返回 gameId。 */
    suspend fun createLocalGame(title: String): Result<String>

    /** 创建人机对局，aiAsWhite = true 时 AI 执白（先手）。返回 gameId。 */
    suspend fun createAiGame(title: String, aiAsWhite: Boolean): Result<String>
     ...
     * [aiAsWhite] 为 null 时人类执当前回合方（轮到谁走谁就是人类方），AI 执另一方。
     */
    suspend fun importFenAsAiGame(title: String, fen: String, aiAsWhite: Boolean? = null): Result<String>

    /** 从 JSON 对战记录导入（含回放走子），返回结构化结果（含导入/跳过手数）。JSON 不合法抛异常。 */
    suspend fun importJson(title: String, json: String): Result<ChessImportResultDto>
