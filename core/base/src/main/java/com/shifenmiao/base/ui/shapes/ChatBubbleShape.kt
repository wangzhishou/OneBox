        ...
        return Outline.Generic(Path().apply {
            ...
            addRoundRect(
                roundRect = RoundRect(
                    left = 0f,
                    top = 0f,
                    right = size.width - arrowSizePx, // leave space for the arrow
                    bottom = size.height,
            ...
            // Draw an equilateral triangle for the arrow of the bubble
            moveTo(size.width - arrowSizePx, arrowPositionPx)
            lineTo(size.width, arrowPositionPx + arrowSizePx)
            lineTo(size.width - arrowSizePx, arrowPositionPx + arrowSizePx * 2)
            close()
        })
    }
}