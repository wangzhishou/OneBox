...
internal class AndroidFileController @Inject constructor(
        ...
        runSuspendCatching {
            ...
            if (settingsState.overwriteFiles) {
                runCatching {
                    ...
                    val fileName = originalUri.getFilename(context).toString()
                ...
                return@withContext SaveResult.Success(
                    ...
                    savingPath = savingPath,
                    fileUri = savingFolder.fileUri.toString(),
                    fileName = filename,
                    treeUri = treeUri
                )
            }
    ...
    override suspend fun writeBytes(
        ...
        runSuspendCatching {
            ...
            val fullPath = uri.getPath(context)
            val fileName = uri.toUri().getFilename(context).toString()
            val savingPath = if (fullPath.endsWith("/$fileName")) {
                fullPath.removeSuffix("/$fileName")
            } else {
                fullPath
            }
            val newMessage = message.ifEmpty {
                getString(
                    R.string.saved_to,
                    savingPath,
                    fileName
            ...
            val treeUri = runCatching {
                ...
                if (documentUri.scheme == "content" && DocumentsContract.isDocumentUri(context, documentUri)) {
                    val docId = DocumentsContract.getDocumentId(documentUri)
                    val treeDocId = docId.substringBeforeLast('/').let {
                        if (it == docId) docId.substringBeforeLast(':') else it
                    }
                    DocumentsContract.buildTreeDocumentUri(documentUri.authority, treeDocId)?.toString()
                } else null
            }.getOrNull()
