...
private class ImagePickerImpl(
    ...
    override fun pickImage() {
        ...
        when (mode) {
            ImagePickerMode.PhotoPickerSingle -> runAction(singlePhotoPickerAction)
            ImagePickerMode.PhotoPickerMultiple -> runAction(multiplePhotoPickerAction)
            ImagePickerMode.CameraCapture -> {
                ContextUtils.requestPermissionAndExecute(
                    context = context,
                    permissions = arrayOf(Manifest.permission.CAMERA),
                    permissionRequest = PermissionRequest.CAMERA,
                    onGranted = { runAction(cameraAction) }
                )
            }
                    ...
                    when (it) {
                        is CameraException -> {
                            AppToastHost.showToast(
                                ...
                                icon = com.t8rin.imagetoolbox.core.resources.Icons.Outlined.LineCameraAlt
                            )
                        }
