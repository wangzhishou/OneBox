 ...
 * @param uri 需要转换的 URI
 * @return 转换后的文件路径，如果转换失败则返回 null
 */
fun Uri.contentUriToFilePath(): String? {
    try {
        if (this.toString().contains("com.android.externalstorage.documents")) {
            // 解析 document ID
            val docId = DocumentsContract.getDocumentId(this)

            // 对于主存储的文档，docId 格式为 "primary:path/to/file"
