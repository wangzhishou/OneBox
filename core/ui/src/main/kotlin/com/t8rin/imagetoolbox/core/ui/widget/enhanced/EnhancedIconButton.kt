...
fun EnhancedIconButton(
    ...
    LocalMinimumInteractiveComponentSize.ProvidesValue(Dp.Unspecified) {
        ...
        val resolvedContainerColor = if (isGlassActive && containerColor != Color.Transparent) {
            Color.Transparent
        } else {
            containerColor
        }
        ...
        OutlinedIconButton(
            ...
            modifier = modifier
                ...
                .then(
                    if (isGlassActive && containerColor != Color.Transparent) {
                        Modifier.glassBackground(
                            style = GlassStyle.Regular,
                            shape = animatedShape,
                            color = containerColor,
                            borderWidth = if (enableAutoShadowAndBorder) settingsState.borderWidth else 0.dp,
                        )
                    } else {
                        Modifier
                    }
            ...
            enabled = enabled,
            border = if (isGlassActive && containerColor != Color.Transparent) {
                null
            } else {
                BorderStroke(
