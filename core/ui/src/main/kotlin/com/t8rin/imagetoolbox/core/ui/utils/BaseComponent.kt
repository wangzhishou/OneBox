...
import com.t8rin.imagetoolbox.core.domain.saving.model.SaveResult.Success
...
abstract class BaseComponent(
    ...
    protected fun registerSave(success: Success? = null) {
    ...
    /**
     * componentScope 使用 Dispatchers.Main.immediate，launch 在主线程上会内联执行。
     *
     * ⚠️ 如果基类 init 中通过 launch 调用了 open/virtual 方法，
     * 子类字段可能尚未初始化。解决方式：
     * - 使用 withContext(Dispatchers.Main) 强制通过 Handler 派发（确保构造器完成）
     * - 或使用 lifecycle.doOnResume 延迟到 UI 可见时执行
     */
