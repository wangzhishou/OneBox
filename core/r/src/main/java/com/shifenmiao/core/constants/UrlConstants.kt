...
object UrlConstants {
     ...
     * 正式发布环境网址
     */
    const val RELEASE_URL = UrlConstantsFlavor.RELEASE_URL

    /**
     * 测试环境网址
     */
    const val DEBUG_URL = UrlConstantsFlavor.DEBUG_URL
    ...
    const val ACCESS_TOKEN = UrlConstantsFlavor.ACCESS_TOKEN

    /**
     * RemoteConfig.accessToken 的完整默认 token，按渠道隔离（国内 / google 各一份）。
     */
    const val REMOTE_CONFIG_ACCESS_TOKEN = UrlConstantsFlavor.REMOTE_CONFIG_ACCESS_TOKEN
     ...
     */
    const val BAIDU_OCR_BASE_URL = UrlConstantsFlavor.BAIDU_OCR_BASE_URL
