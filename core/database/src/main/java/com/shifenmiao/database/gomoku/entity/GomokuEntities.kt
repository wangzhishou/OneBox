package com.shifenmiao.database.gomoku.entity
...
@Entity(tableName = "gomoku_game")
data class GomokuGameEntity(
...
@Entity(
    tableName = "gomoku_ply",
    indices = [
        Index(value = ["game_id", "ply"], unique = true),
        Index(value = ["game_id"]),
    ],
)
data class GomokuPlyEntity(
    @PrimaryKey val id: String,
...
@Entity(
    tableName = "gomoku_ai_task",
    indices = [Index(value = ["game_id", "status"])],
)
data class GomokuAiTaskEntity(
    @PrimaryKey val id: String,
    @ColumnInfo(name = "game_id") val gameId: String,
