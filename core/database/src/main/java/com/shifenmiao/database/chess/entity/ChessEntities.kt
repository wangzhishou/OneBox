package com.shifenmiao.database.chess.entity
...
data class ChessPlyEntity(
    @PrimaryKey val id: String,
    @ColumnInfo(name = "game_id") val gameId: String,
    val ply: Int,
    /** UCI 坐标着法(如 e2e4);列名与象棋对齐沿用 move_ucci */
...
@Entity(
    tableName = "chess_ai_task",
    indices = [Index(value = ["game_id", "status"])],
)
data class ChessAiTaskEntity(
    @PrimaryKey val id: String,
    @ColumnInfo(name = "game_id") val gameId: String,
