...
data class CodeBlockStyle(
    ...
    companion object {
        val Default: CodeBlockStyle = CodeBlockStyle()
    }
}
...
internal val DefaultCodeBlockBackgroundColor: Color = AppTheme.colorScheme.surfaceContainerHighest
...
fun RichTextScope.FencedCodeBlock(
    astFencedCodeBlock: AstFencedCodeBlock,
    wordWrap: Boolean? = false
) {
    CodeBlock(wordWrap = wordWrap) {
        if (astFencedCodeBlock.annotatedString?.isNotBlank() == true) {
            BasicText(
                text = astFencedCodeBlock.annotatedString!!
            )
        } else {
            Text(astFencedCodeBlock.literal)
        }
    }
}
 ...
 */
@Composable
internal fun RichTextScope.CodeBlockLayout(
    wordWrap: Boolean,
