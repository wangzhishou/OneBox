...
```bash
.venv/bin/python fetch_images.py              # 拉线上图(只读)
.venv/bin/python push_images.py --dry-run     # 预览差异
.venv/bin/python push_images.py [--locale de-DE]  # 正式推
```
...
.venv/bin/python fetch_listing.py   # 只读抓取,不会改动线上
```
...
```bash
.venv/bin/python push_listing.py --dry-run            # 预览差异
.venv/bin/python push_listing.py [--locale es-ES]     # 正式推
```
