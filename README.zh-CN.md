...
- **悬浮机器人,不必干等**:Agent 任务要跑一阵?直接退出 App 去做别的——任务在后台继续执行,桌面悬浮机器人可拖至任意边缘停靠,完成后点一下即可回到任务结果。不用再盯着加载圈等待。(国内渠道版本提供,需授予悬浮窗权限)
- **一个入口,多家模型**:一个对话入口接入多家主流大模型,支持自定义 AI 引擎与提示词管理。
- **自由接入,成本自主**:支持配置第三方 API 密钥,自由选择大模型服务——接入免费 API 即可零成本扩展 AI 能力。
